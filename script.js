...
function addListeners() {
  ...
  client.on('clearchat', channel => {
    ...
    showAdminMessage({
      message: `Chat wurde gecleart!`, // ${chan}
      timeout: 10000 });
  });

  client.on('emoteonly', (channel, on) => {
    if(on === true){
    showAdminMessage({
      message: `Emoteonly wurde aktiviert`, // ${chan}
      timeout: 10000 })
    }
    else{
    showAdminMessage({
      message: `Emoteonly wurde deaktiviert`, // ${chan}
      timeout: 10000 })
    }
  });
...
client.on('messagedeleted', (channel, username, deletedMessage, userstate) => {
    msgid = userstate["target-msg-id"];
    console.log(msgid);
    removeChatLine({ channel, msgid});
    });
...
function removeChatLine(params = {}) {
  ...
  join('');
  paratime = {timeout: 1000};
  chatEle.querySelectorAll(search).forEach(n =>
...
function showMessage({ chan, type, message = '', data = {}, timeout = 0, attribs = {} } = {}) {
  ...
  console.log(data.id);
  chatLine_.setAttribute('msgid', data.id);
  if (chan) {
  ...
  if (type === 'chat') {
    if((data.username != chatbotname && message.startsWith(chatprefix) === false) || (message.startsWith("!hype") || message.startsWith("!HYPE"))){


      if((message.startsWith("!hype") || message.startsWith("!HYPE")) && hype_activate == true){
        ...
        // go Buckeyes!
        var colors = ['#fc4040', '#f55f5f'];
        
        confetti({
          angle: randomInRange(40, 0),
          spread: randomInRange(1, 1),
          decay: 0.8,
          particleCount: 1,
          gravity: -0.5,
          colors: colors,
          origin: { y: 0.98, x: randomInRange(0.0, 0.1)}
        });
